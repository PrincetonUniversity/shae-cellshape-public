...
function Cell_shape_detector3dConvFinalFolder(folderName,folderIdx,fileGroupSize);
% %%rm
% if nargin<1
% % image folder
...
for kkFile = 1:numel(folderIdx);
    ...
    try
        ...
        hyper_stack=pedistalSubtract(hyper_stack,'z3');
        ...
        if flag.bpbFlag2
            ...
            [xyzs,zbw,zsq,r_init]=cellCenterline3d(hyper_stack,flag.z_level,window,...
                image_para.imsize(3),flag);
            cline_para.radius=r_init;
            cline_para.pts=ceil(2*pi*r_init/4)*4;
        end
 ...
 %        cline_para.beta=cline_para.beta/10;
        ...
        %% Load PSF
        CLscale = 4;
        [x,y,z,eg] = activeSurfaceFit(x1,y1,z1,hyper_stack,...
            cline_para,image_para,cline_para.upFactor,flag);
        % it would be good if there was some check pointing being done,
        % like save along the way in case a job gets killed due to time
        ...
        if flag.F1==1         
            for tt=1:image_para.Fstack_t_size
                ...
                %membraneFraction
                [mFraction,Fmembrane]=membraneFraction(x,y,z,F1,image_para,hyper_stack,cline_para.gradflag);
...
%               r_steps=linspace(.8,1.2,5);
               r_steps=reshape(r_steps,1,1,[]);  
                ...
                if Fim_size(3)~=image_para.imsize(3)
                    ...
                    Ffr2=interp3(Ygrid,Xgrid,Zgrid,Fmembrane,rky,rkx,rkz);                    
                else
                    Ffr=interp3(F1,rky,rkx,rkz);
                    Ffr2=interp3(Fmembrane,rky,rkx,rkz);                    
               ...
               Ffr2=FintensityAngleCorrection(x,y,z,Ffr2);

                
                % Correction for Z intensity Loss
                ...
                if isfield(flag,'polyFit');
                    if flag.polyFit
                        ...
                        polyFit = PolymerFit(Fmembrane, x,y,z,Ffr2,cline_para,image_para );
                    else
                        polyFit=[];
                    end
                else
        ...
        if isfield(flag,'polyFit');
            if flag.polyFit
                for iPoly=1:length(polyFit);
                    ...
                    polyFit(iPoly).Fluor=polymerCurvatureLocalization( Ff2,polyFit(iPoly).rc );
                end
        ...
        if flag.F1
            cellF=struct('Ff',Ff,'Ffr',Ffr,'I',I,'Ir',Ir,'Irdf',Irdf,...
                'Frdf',Frdf,'Ff2',Ff2,'Ffr2',Ffr2,'mFraction',mFraction);
        else
            cellF=[];
        end
        ...
        if flag.profile == 2
            profileStats = profile('info');
            save(p2,'profileStats','-append')
