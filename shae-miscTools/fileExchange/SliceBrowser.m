...
%>       - Pressing 'w' switches the display of the crosshairs on and off.
%>   Example of usage:
%>       load mri.dat
%>       volume = squeeze(D);
%>       SliceBrowser(volume);
...
% Modified by JN 20150128, fix bug that errored when data in a given slice
% is constant. 
% Modified by BPB 20150803, add 'w' to switch the crosshair on and off
% ======================================================================
...
switch curr_char
    ...
    case 119 % 'w'
        handles.cross_visible = 1 - handles.cross_visible;
    case 30
...
sp1 = subplot(2,2,1);
%colorbar;
clims(clims==0)=.00000001;
if diff(clims)==0;
    clims=clims.*[.99999,1.00001];
end
imagesc(sliceXY, clims);
title('Slice XY');
ylabel('X');xlabel('Y');
if handles.cross_visible
