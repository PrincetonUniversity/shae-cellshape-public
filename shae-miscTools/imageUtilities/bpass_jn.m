...
% 20141210 bpb adding support for nans, replace them with the 6 connected
% nanmedian for filtering and then after filtering make them nan again

isnanPositions = find(isnan(image_array));
image_array = inpaint_nans(image_array); 
...
if numel(isnanPositions)>0;
    for iPosition = 1:numel(isnanPositions)
            % make a mask of each of these
       res(isnanPositions(iPosition)) = nan;
    end
end
