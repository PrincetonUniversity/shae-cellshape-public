function [savedPeaks] = find3DPeaksAtSurface3(folderName,param)
if nargin<1
    %% open an image
...
[Ymap,Xmap,Zmap]=meshgrid(-param.xyPSFsize:param.xyPSFsize,-param.xyPSFsize:param.xyPSFsize,...
    -param.zPSFsize:param.zPSFsize);
...
if isempty(zPos)
    ...
    sliceSize='0.1';
    warning('find3DpeaksAtSurface3:fixedZstep','Step set at 0.1');
...
for iPeak = 1:size(peaksOut,1);
    ...
    if and(and(abs(bestEstBren-bestEstSum)<param.similarBestEst,...
        ...
        bestEstP = ((bestEst-zPos(1))/(zPos(2)-zPos(1))+1);
        
        
        subImageIn =     tempImage2(round(peaksOut(iPeak,2)-param.pkSize):...
            round(peaksOut(iPeak,2)+param.pkSize),...
            round(peaksOut(iPeak,1)-param.pkSize):...
            round(peaksOut(iPeak,1)+param.pkSize),...
            round(bestEstP-param.avgZForXY):...
            round(bestEstP+param.avgZForXY));
        ...
        newCentPixX = (xCntrd/(1000*param.pixSize));
        newCentPixY = (yCntrd/(1000*param.pixSize));
        ...
        if newCentPixX<param.xyPSFsize+1 || newCentPixX> size(tempImage,2)-param.xyPSFsize || ...
            ...
            try
                ...
                if param.isInterp3D
                    ...
                    psfExample(psfCounter).psfCutOut=interp3(tempImage,...
                        Xmap+newCentPixX,Ymap+newCentPixY,Zmap+bestEstP,'*linear',NaN);
                    
                else
                    psfExample(psfCounter).psfCutOut = tempImage(...
                       round( newCentPixY-param.xyPSFsize):...
                       round( newCentPixY+param.xyPSFsize) ,...
                       round( newCentPixX-param.xyPSFsize):...
                       round( newCentPixX+param.xyPSFsize) ,...
                       round( bestEstP-param.zPSFsize):...
                       round( bestEstP+param.zPSFsize));
                    
                end
                
            % normalize to peak intensity
            psfExample(psfCounter).psfBkgd = nanmedian(psfExample(psfCounter).psfCutOut(:));
            psfExample(psfCounter).psfCutOut = psfExample(psfCounter).psfCutOut - psfExample(psfCounter).psfBkgd;
            psfExample(psfCounter).psfPeak = nanmax(psfExample(psfCounter).psfCutOut(:));
...
if not(exist('sliceSize','var'))
   sliceSize = 0.075;
   warning('find3DPeaksAtSurface:sliceSize','Slice size missing');
end
try
