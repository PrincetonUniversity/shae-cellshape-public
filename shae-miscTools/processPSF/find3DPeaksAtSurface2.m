function find3DPeaksAtSurface(folderName,param)
if nargin<1
    %% open an image
...
if nargin<1
    ...
    param.pixSize = 0.080; % xy pixel size in um
    param.kernelZ = 0.1; % Gaussian smooth along focal dimension in um
    
    %     % peak determination
    param.pkThresh = 700; % thresh for pkfnd
...
for iPeak = 1:size(peaksOut,1);
    ...
    if and(and(abs(bestEstBren-bestEstSum)<param.similarBestEst,...
        ...
        botpad=max(-bestEstP+param.zPSFsize+1,0);
        toppad=max(bestEstP- size(tempImage,3)+param.zPSFsize,0);
            ...
            tempImage3= tempImage(newCentPixY-param.xyPSFsize:newCentPixY+param.xyPSFsize ,...
                newCentPixX-param.xyPSFsize:newCentPixX+param.xyPSFsize ,:);
            
            tempImage3=padarray(tempImage3,[0,0,botpad],NaN,'pre');
            tempImage3=padarray(tempImage3,[0,0,toppad],NaN,'post');

            psfExample(psfCounter).psfCutOut=...
                tempImage3(:,:,bestEstP-param.zPSFsize+botpad:bestEstP+botpad+param.zPSFsize);
...
save(saveFile,'folderName','savedPeaks','param','firstStagePos','stagePosDif','psfExample');
disp(folderName);
disp(saveFile);
% disp(thresh);
disp([]);
