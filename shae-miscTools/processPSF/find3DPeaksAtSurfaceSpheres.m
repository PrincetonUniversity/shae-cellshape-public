...
% param.circleRadRange = [7,15];
[peaksOut,radii,metric] = imfindcircles(std(tempImage2,0,3),param.circleRadRange);
...
%     
    ...
    options.intensityMode = 'edgeSlope';
    [bestEstSlope,outputSlope] = ZPeakEstimator(pkCutOut,param,zPos,options);
    
    % store results
    peaks2(iPeak,5) = bestEstBren;
    peaks2(iPeak,6) = outputBren.isKept;
    peaks2(iPeak,7) = outputBren.fwhmZ;
    peaks2(iPeak,8) = bestEstSlope;
    peaks2(iPeak,9) = outputSlope.isKept;
    peaks2(iPeak,10) = outputSlope.fwhmZ;
% %     
